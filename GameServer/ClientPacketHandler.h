...
public:
	static void	Init()
	{
		...
		GPacketHandler[PKT_C_ENTER_ROOM] = [](std::function<void()>& outFunc, PacketSessionRef& session, BYTE* buffer, int32 len) {
			return GetCallback<Protocol::C_ENTER_ROOM>(outFunc, Handle_C_ENTER_ROOM, session, buffer, len);
		};
		GPacketHandler[PKT_C_CHAT] = [](std::function<void()>& outFunc, PacketSessionRef& session, BYTE* buffer, int32 len) {
